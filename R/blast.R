...
#' \code{makeblastdb}. For a list of options, run \code{makeblastdb -help}.
...
#' @export
build_blast_db <- function (in_seqs, out_name, db_type = "nucl",
                            other_args = NULL, ...) {

  # modify arguments
  arguments <- c(paste("-in", in_seqs),
                 paste("-out", out_name),
                 paste("-dbtype", db_type),
                 other_args)
